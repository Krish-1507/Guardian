// ==============================================================================
// Guardian permanent repro test — finding ledger-a97b7d00
// Linked from .guardian/scan-latest.json (issue / cluster finding id ledger-a97b7d00).
//
// Scenario: duplicate-webhook on /api/webhooks/razorpay
//   order ord_webhook_3 · idempotency key idem_webhook_3
//   Gateway contract: exactly ONE charge for key "pay_webhook_3"
//   (observed now: 2 charges — the bug this test exists to guard).
//
// Run via: npx guardian-cli repro ledger-a97b7d00
...
const SCENARIO = "duplicate-webhook";
const ORDER_ID = "ord_webhook_3";
const IDEM_KEY = "idem_webhook_3";
const EXPECTED_KEY = "pay_webhook_3";
const AMOUNT = 1000;
const CURRENCY = "INR";
...
test("guardian repro ledger-a97b7d00: exactly one gateway charge for key '" + EXPECTED_KEY + "'", async () => {
  assert.ok(PRELOAD, "GUARDIAN_LEDGER_PRELOAD not set — run this via `npx guardian-cli repro ledger-a97b7d00`");
