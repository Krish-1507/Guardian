 ...
 * other Guardian artifact) and lookup helpers for `guardian repro` / inspect.
 */
...
export function persistPen(repo: string, result: PenResult): { file: string } {
  const sealed = seal(result, `guardian pen result for ${repo}`);
...
export function loadPenLatest(repo: string): PenResult | null {
  const p = path.join(repo, ".guardian", "pen-latest.json");
